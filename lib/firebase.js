...
import { getFirestore, collection, addDoc, query, where, onSnapshot, deleteDoc, doc, updateDoc, serverTimestamp, getDocs, setDoc } from 'firebase/firestore'
...
export const saveChat = async (userId, chatData) => {
  try {
    ...
    // Use doc() with explicit ID to prevent Firestore from auto-generating
    const chatRef = doc(collection(db, 'chats'), chatData.id)
    ...
    console.log('Chat saved with ID:', chatData.id)
    return chatData.id
  } catch (error) {
