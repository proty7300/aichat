...
// ── Pollinations Image Generation (FREE, no API key needed) ──
export async function generateImagePollinations({ prompt, model = 'flux' }) {
  const encodedPrompt = encodeURIComponent(prompt)
  const seed = Math.floor(Math.random() * 999999)
  const url = `https://image.pollinations.ai/prompt/${encodedPrompt}?model=${model}&width=1024&height=1024&seed=${seed}&nologo=true`

  // Pollinations langsung return image URL — tidak perlu fetch API
  // Cukup return URL-nya, lalu tampilkan sebagai img tag
