...
export async function generateImageGoogle({ prompt, apiKey }) {
  // Use v1beta endpoint with correct model name for Imagen 3
  const response = await fetch(
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({}))
    console.error('Google Imagen error response:', response.status, err)
    throw new Error(err.error?.message || `Imagen API error ${response.status}`)
  }
  
  const data = await response.json()
  console.log('Google Imagen response:', data)
