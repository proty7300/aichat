...
// ── Anthropic ──
async function callAnthropic({ apiKey, model, systemPrompt, messages, stream, baseUrl }) {
  const endpoint = baseUrl ? `${baseUrl}/v1/messages` : 'https://api.anthropic.com/v1/messages'
  console.log('Calling Anthropic endpoint:', endpoint, 'model:', model, 'stream:', stream)
...
async function callAnthropicNonStreaming({ apiKey, model, systemPrompt, messages, baseUrl }) {
  const endpoint = baseUrl ? `${baseUrl}/v1/messages` : 'https://api.anthropic.com/v1/messages'
  console.log('Calling Anthropic (non-streaming) endpoint:', endpoint, 'model:', model)
  const response = await fetch(endpoint, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
export async function streamChat({ providerId, model, modeId, messages, overrideKey }) {
  ...
  // Freemodel Claude doesn't support streaming - use non-streaming mode
  if (providerId === 'freemodelclaude') {
    return callAnthropicNonStreaming({ apiKey, model, systemPrompt, messages, baseUrl })
  }
  if (providerId === 'anthropic') {
    return callAnthropic({ apiKey, model, systemPrompt, messages, stream: true, baseUrl })
  }
