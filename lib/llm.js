...
export async function callCloudflareVision({ prompt, imageBase64, model, apiToken, accountId, systemPrompt }) {
  ...
  // Auto-agree to Llama license if required
  const agreeRes = await fetch(url, {
    method: 'POST',
    headers,
    body: JSON.stringify({ messages: [{ role: 'user', content: 'agree' }], stream: false }),
  })
  const agreeData = await agreeRes.json().catch(() => ({}))
  const needsAgree = agreeData.errors?.some((e) => e.message?.includes('Model Agreement'))
  if (needsAgree) {
    // Already sent agree above, Cloudflare registers it — proceed
    console.log('Cloudflare Llama: agreement sent')
  }
